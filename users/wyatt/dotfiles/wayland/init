...
wlr-randr --output HDMI-A-1 --pos 900,1440
wlr-randr --output DP-2 --pos 3440,0
riverctl focus-follows-cursor normal
riverctl spawn kile
riverctl border-width 4
defaultLayout="all ( { v: ( { v: v h } 1 .625 0 ) d } 1 .7 0 )"
for i in $(seq 1 3); do
	riverctl send-layout-cmd kile "order ascend"
	# riverctl send-layout-cmd kile "yoffset 60"
	riverctl send-layout-cmd kile "all ( { v: ( { v: v h } 1 .625 0 ) d } 1 .7 0 )"
	riverctl send-layout-cmd kile "outer_padding 15"
